...
export default function QuizPage() {
  ...
  const [answeredRecord, setAnsweredRecord] = useState<Record<number, { selected: string, isCorrect: boolean }>>({});
  ...
  useEffect(() => {
    if (questions.length > 0) {
      localStorage.setItem(`distillprep_${topic}_state`, JSON.stringify({
        answeredRecord,
        currentIndex,
      }));
    }
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#0f172a] text-slate-100 flex items-center justify-center font-sans tracking-wide">Loading...</div>;
  }
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex flex-col font-sans text-slate-100">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-slide-up">
          ...
          <p className="text-slate-400">We are currently building `{topic}`. Check back soon.</p>
        </div>
      </div>
    );
  }
  ...
  const handleSelectOption = (key: string) => {
    if (isAnswered) return; // Disable multiple attempts
            ...
            {isAnswered && (
              <div className="mt-12 pt-8 border-t border-slate-800 animate-slide-up">
                <h4 className="text-sm font-semibold tracking-wider text-slate-500 uppercase mb-4 flex items-center gap-2 relative">
                  <div className={`w-2 h-2 rounded-full ${currentAnswerState.isCorrect ? 'bg-emerald-500' : 'bg-red-500'}`} />
                  Explanation
                </h4>
                
                {/* Specific typography requirements applied here */}
                <div 
                  className="text-sm text-gray-400 leading-relaxed space-y-4 font-normal"
                  dangerouslySetInnerHTML={{ __html: parseMarkdownToHTML(currentQ.explanation) }}
                />
